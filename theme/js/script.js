...
(function(window, $, undefined) {
    ...
    MARTY.init = function() {
        ...
        MARTY.events.onPopState();

        // Show view pane
        if (window.location.pathname !== '' && window.location.pathname !== '/') {
            setTimeout(function() {
                MARTY.events['onKeyTop']();
            }, 250);
        }

        if (MARTY.touch == false) {
            // Arrow keypress
            $window.keydown(function(event) {
                switch (event.which) {
                    case 38:
                        ...
                        break;
                    case 39:
                        MARTY.events['onKeyRight']();
                        break;
                    case 40:
                        MARTY.utils.hideAboutPane();
                    case 27:
                        MARTY.events['onKeyBottom']();
                        break;
                    case 37:
                        MARTY.events['onKeyLeft']();
                        break;
                }
            });
        }
        ...
        if (MARTY.touch == true) {
            ...
            $(".slide").touchwipe({
                ...
                wipeUp: function() {
                    MARTY.events['onKeyDown']();
                },
                wipeDown: function() {
                    MARTY.events['onKeyUp']();
                },
                min_move_x: 20,
                min_move_y: 20,
                preventDefaultEvents: true
        ...
        if (MARTY.mobile == true) {
            // start orientation listeners
            MARTY.events.onOrientation();
            $window.bind('orientationchange', MARTY.events.onOrientation);
        }

        // Arrow clicks
        $document.on('click', '#keys a', function(event) {
            event.preventDefault();
            MARTY.events['on' + $(this).attr('id')]();
        });
    ...
    MARTY.events = {
        onResize: function() {
            MARTY.utils.setTickerWidth();
            MARTY.utils.setTickerPosition();
        },
        ...
        onPopState: function(force) {
            ...
            if (MARTY.inited || force) {
                MARTY.utils.togglePanes();
                MARTY.utils.setCurrentSlide(MARTY.utils.getView());
                MARTY.utils.redrawView();
            } else {
                MARTY.inited = true;
            }
        },
        onKeyTop: function(view) {
            if ($('#about-pane').hasClass('show')) {
                ...
                MARTY.utils.hideAboutPane();
                MARTY.utils.showPanes();
                MARTY.paths.push({
                    'title': view,
                    'path': '/' + view
                });
                MARTY.utils.flashKey('KeyTop');
            }
        ...
        onKeyRight: function() {
            MARTY.utils.nextSlide();
            MARTY.utils.redrawView();
            MARTY.utils.flashKey('KeyRight');
        ...
        onKeyBottom: function() {
            if ($('#panes').hasClass('show')) {
                MARTY.utils.hidePanes();
                MARTY.paths.push({
                    'title': 'home',
                    'path': '/'
                });
                MARTY.utils.flashKey('KeyBottom');
            } else {
                MARTY.utils.toggleAboutPane();
            }
        ...
        onKeyLeft: function() {
            MARTY.utils.prevSlide();
            MARTY.utils.redrawView();
            MARTY.utils.flashKey('KeyLeft');
            if ($('#panes').hasClass('show')) {
                var view = $('.current').data('view');
                MARTY.paths.push({
                    'title': view,
    ...
    MARTY.utils = {
        setTickerWidth: function() {
            $('#ticker-moment').css({
                width: MARTY.utils.getTickerWidth
            });
        },
        ...
        setTickerPosition: function() {
            ...
            var newPosition = (($('.slide.current').index()) * MARTY.utils.getTickerWidth()) + leftGutter;
            $('#ticker-moment').css({
                left: newPosition + 'px'
            });
        },
        setCurrentPane: function(view) {
            MARTY.utils.setCurrentTitle(view);
            $('.pane').css({
                display: 'none'
            });
            $('.pane[data-view="' + $('.slide.current').data('view') + '"]').css({
                display: 'block'
            });
        },
        ...
        nextSlide: function() {
            $slides.each(function() {
                ...
                if (x == 1) {
                    MARTY.utils.setCurrentSlide($(this).data('view'));
                }
            });
        },
        prevSlide: function() {
        ...
        getView: function() {
            if (!window.location.pathname || window.location.pathname === '/') {
                ...
                return MARTY.paths.split()[1];
            }
        },
        redrawView: function() {
            MARTY.utils.setCurrentPane();
            MARTY.utils.setCurrentDate();
            MARTY.utils.setTickerPosition();
            MARTY.utils.setTickerWidth();
        }
    };
